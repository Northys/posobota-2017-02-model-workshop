...
sudo apt-get update
sudo apt-get install -y ansible python

# Setup Ansible for Local Use and Run
cp ./inventories/dev /etc/ansible/hosts -f
chmod 666 /etc/ansible/hosts
cat ./files/authorized_keys >> /home/vagrant/.ssh/authorized_keys
sudo ansible-playbook ./playbook.yml -e hostname=$1 --connection=local
